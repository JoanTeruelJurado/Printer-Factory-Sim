...
import { Factory, Store, Truck, Play, Pause, RotateCcw } from 'lucide-react';
...
const SimDashboard = ({ gameState, onRefresh, onToast }) => {
  ...
  const WalletChart = ({ data }) => {
    ...
    const allVals = data.flatMap(d => [d.mfg_wallet ?? 0, d.ret_wallet ?? 0]);
    ...
    const mfgPoints = data.map((d, i) => `${xScale(i)},${yScale(d.mfg_wallet ?? 0)}`).join(' ');
    const retPoints = data.map((d, i) => `${xScale(i)},${yScale(d.ret_wallet ?? 0)}`).join(' ');
    ...
    return (
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ maxHeight: '200px' }}>
        ...
        {/* Lines */}
        <polyline points={mfgPoints} fill="none" stroke="#22c55e" strokeWidth="2" />
        <polyline points={retPoints} fill="none" stroke="#a855f7" strokeWidth="2" />
        {/* Dots */}
        {data.map((d, i) => (
          <g key={i}>
            <circle cx={xScale(i)} cy={yScale(d.mfg_wallet ?? 0)} r="3" fill="#22c55e" />
            <circle cx={xScale(i)} cy={yScale(d.ret_wallet ?? 0)} r="3" fill="#a855f7" />
          </g>
        ))}
        {/* Legend */}
        <rect x={width - 140} y={5} width="10" height="10" fill="#22c55e" rx="2" />
        <text x={width - 126} y={14} className="fill-gray-400" fontSize="10">Manufacturer</text>
        <rect x={width - 140} y={20} width="10" height="10" fill="#a855f7" rx="2" />
        <text x={width - 126} y={29} className="fill-gray-400" fontSize="10">Retailer</text>
      </svg>
    );
  };
  ...
  const FulfillmentChart = ({ data }) => {
    ...
    const maxVal = Math.max(1, ...data.map(d => (d.fulfilled ?? 0) + (d.backordered ?? 0)));
    const barW = Math.max(8, Math.min(30, innerW / data.length - 4));
    ...
    return (
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ maxHeight: '200px' }}>
        ...
        {data.map((d, i) => {
          ...
          const cx = xScale(i);
          const fulfilledH = (fulfilled / maxVal) * innerH;
          const backorderedH = (backordered / maxVal) * innerH;
          return (
            <g key={i}>
              {/* Fulfilled (green) on bottom */}
              <rect
                x={cx - barW / 2}
                y={padding.top + innerH - fulfilledH - backorderedH}
                width={barW}
                height={fulfilledH}
                fill="#22c55e"
                rx="2"
              />
              {/* Backordered (red) stacked on top */}
              {backordered > 0 && (
                <rect
                  x={cx - barW / 2}
                  y={padding.top + innerH - backorderedH}
                  width={barW}
                  height={backorderedH}
                  fill="#ef4444"
                  rx="2"
              ...
              {/* X label */}
              <text x={cx} y={height - 5} textAnchor="middle" className="fill-gray-400" fontSize="10">
                {d.day}
              </text>
            </g>
          );
        ...
        {/* Legend */}
        <rect x={width - 130} y={5} width="10" height="10" fill="#22c55e" rx="2" />
        <text x={width - 116} y={14} className="fill-gray-400" fontSize="10">Fulfilled</text>
        <rect x={width - 130} y={20} width="10" height="10" fill="#ef4444" rx="2" />
        <text x={width - 116} y={29} className="fill-gray-400" fontSize="10">Backordered</text>
      </svg>
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 flex flex-wrap items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          <span className="px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-full text-sm font-bold">
            Day {turn?.day ?? dashState?.manufacturer?.day ?? '-'}
          </span>
        </div>
      </div>

      {/* Section 2: Scenario Events Banner */}
            ...
            <div className="flex flex-wrap gap-2">
              {scenarioInfo.active_events.map((evt, i) => (
                <div key={i} className="flex items-center gap-2 px-3 py-2 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg">
                  ...
                  {evt.description && (
                    <span className="text-xs text-orange-600 dark:text-orange-400">{evt.description}</span>
                  )}
                </div>
              ))}
            </div>
      ...
      {/* Section 3: Three-App Status Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden mt-1">
                <div
                  ...
                  style={{ width: `${Math.min(100, (sumValues(prov?.stock) / 4000) * 100)}%` }}
                />
              </div>
            </div>
            <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border-t-4 border-green-500">
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className={`text-2xl font-bold ${walletColor(mfg?.wallet)}`}>
                {formatCurrency(mfg?.wallet ?? 0)}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all ${(mfg?.utilisation ?? 0) > 0.8 ? 'bg-red-500' : (mfg?.utilisation ?? 0) > 0.5 ? 'bg-yellow-500' : 'bg-green-500'}`}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border-t-4 border-purple-500">
          ...
          <div className="space-y-3">
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">Wallet</p>
              <p className={`text-2xl font-bold ${walletColor(ret?.wallet)}`}>
                {formatCurrency(ret?.wallet ?? 0)}
              </p>
            </div>
            <div>
              ...
              {(!ret?.stock || Object.keys(ret.stock).length === 0) && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <WalletChart data={history} />
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Fulfillment Over Time</h3>
